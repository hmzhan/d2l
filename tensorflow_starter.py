...
weights = [
	tf.Variables(tf.random_normal([112, 50], seed = 888)),
	tf.Variables(tf.random_normal([50, 2], seed = 888))
]

biases = [
	tf.Variables(tf.random_normal([50], seed = 888)),
	tf.Variables(tf.random_normal([2], seed = 888))
]

pred = multilayer_perceptron(x, weidhts, biases)

logits = tf.nn.softmax(pred)

loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_withlogits(pred, y))
